...
import pandas as pd
import json,sys
from utils.graphviz.GraphvizTree import GraphvizTree
...
class Spiredf():
    ...
    def overlap(self, other = None):
        return set(self.df[Spiredf.ptid]).intersection(set(other[Spiredf.ptid]))
    
    def find_children(self):
        #
        for i in Spiredf.instances.values():
            #logger.debug(i)
            if i.name!=self.name and i not in self.children:
                if len(i.df)<len(self.df):
                    self_overlap=i.overlap(self.df)
                    logger.debug(f"overlap:{self_overlap}")
                    if len(self_overlap) > 0:
                        for ch in self.children:
                            child_overlap=ch.overlap(self.df)
                            if i.name!=ch.name and self_overlap.issuperset(child_overlap) and i not in self.children:
                                self.children.remove(ch)
                                self.children.append(i)
                                logger.debug(f"replaced {ch.name} with {i.name}")
                        
                        if  i not in self.children and  self.is_any_child_superset(i):
                            self.children.append(i)
                            logger.debug(f"appended {i.name} to {self.name}")
                            
    def is_any_child_superset(self,i):
        self_overlap=i.overlap(self.df)
        for ch in self.children:
            child_overlap=ch.overlap(self.df)
            if i.name!=ch.name and \
                child_overlap.issuperset(self_overlap):
                return False
        return True
    @classmethod
    def find_all_children(cls):#
    ...
    def get_links(self,parent):
        if isinstance(parent,Spiredf):
            links=[(parent.name,self.name,set(self.df[Spiredf.ptid]))]
        else:
            links=[('root',self.name,set(self.df[Spiredf.ptid]))]
        for ch in self.children:
            links=links+ch.get_links(self)
        return links
    
    def draw_child_tree(self):
        g=GraphvizTree()
        for i in self.get_links(None):
            g.add_link(str(i[0]),str(i[1]),f"{str(i[1])}\n{str(i[2])}")
        return g.draw_flow_graphviz()

    def draw_parent_tree(self):
        g=GraphvizTree()
        for i in self.get_parent_links(self):
            logger.debug(f"link:{str(i[0])} {str(i[1])} {str(i[1])} {str(i[2])}")
            g.add_link(str(i[0]),str(i[1]),f"{str(i[1])}\n{str(i[2])}")
        return g.draw_flow_graphviz()
    
    def get_parent_links(self,child):
        logger.info(f" in {self.name}")
        if len(self.parents)==0:
            return [('root',self.name,set(self.df[Spiredf.ptid]))]
        else:
            links=[]
            for p in self.parents:
                links=links+p.get_parent_links(self)+[(p.name,self.name,set(self.df[Spiredf.ptid]))]
...
#Spiredf.reset()
#test_parent_tree()
#[str(Spiredf.instances[x]) for x in Spiredf.instances]
